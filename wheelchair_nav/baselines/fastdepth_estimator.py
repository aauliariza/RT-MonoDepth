"""Thin inference wrapper around the UNMODIFIED FastDepth network already
shipped in this repo (networks/FastDepth/model.py: MobileNetSkipAdd,
MobileNet encoder + depthwise-separable skip-add decoder). Used only as an
apples-to-apples comparison baseline against RT-MonoDepth -- see
scripts/train_fastdepth_sunrgbd.py and evaluation/eval_depth_comparison.py.

MobileNetSkipAdd's final layer is a plain Conv+BatchNorm+ReLU (see
`pointwise()` in the model file), so its raw output is already >= 0 -- no
sigmoid/disparity remapping is added here, only a clamp into the same
indoor metric range RT-MonoDepth is trained/evaluated over, applied
identically at both training and inference time.
"""
...
class FastDepthEstimator:
    """Loads a FastDepth checkpoint saved by train_fastdepth_sunrgbd.py and
    produces a metric depth map in meters for a single BGR frame.
    """
    ...
    def __init__(
        ...
        weights_path = os.path.join(weights_dir, "fastdepth.pth")
        ckpt = torch.load(weights_path, map_location=self.device)
        self.feed_height = int(ckpt["height"])
        self.feed_width = int(ckpt["width"])

        self.model = MobileNetSkipAdd()
