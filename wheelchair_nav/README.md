...
3. (Opsional, untuk fine-tuning YOLO26-nano di langkah 3) sekaligus konversi anotasi
   ...
   Jika hasil konversi terlalu sedikit, langsung pakai opsi 1 di langkah 3
   (fine-tune dari bobot COCO-pretrained) tanpa label SUN RGB-D.

---

## 2. Training RT-MonoDepth (full, from scratch) di SUN RGB-D
...
## 3. Training / fine-tuning YOLO26-nano (deteksi bbox, class-agnostic)
...
**Opsi A (disarankan): fine-tune dari bobot COCO-pretrained**
...
**Opsi B: training dari bobot acak (tanpa COCO)**
...
(COCO-pretrained) langsung bisa dipakai apa adanya di langkah 4 tanpa fine-tuning --
sistem navigasi tetap berjalan class-agnostic karena nama kelas dibuang.

---

## 4. Testing sistem navigasi pada file video
...
### 5a. Depth: akurasi, error, jumlah parameter, FPS
...
### 5b. Deteksi obstacle: mAP, precision, recall, parameter, FPS
...
### 5c. Sistem navigasi end-to-end: FPS pipeline, keamanan, akurasi keputusan, error jarak
...
- Parsing anotasi 2D SUN RGB-D (`annotation2Dfinal/index.json`) bersifat best-effort
  ...
  dari bobot COCO-pretrained (Opsi A langkah 3) tidak bergantung pada langkah ini.
